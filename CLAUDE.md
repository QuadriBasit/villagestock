...
## UI form controls (hard rule)

Never use raw HTML form elements in app UI (`<input>`, `<select>`, `<textarea>`, native date/time pickers, etc.). Always use `@/components/ui/` — `Input`, `Select`, `DatePickerField`, `DateTimeField`, `CurrencyInput`, `Checkbox`, `Button`, and related shadcn wrappers. See `.cursor/rules/no-raw-form-controls.mdc`.
