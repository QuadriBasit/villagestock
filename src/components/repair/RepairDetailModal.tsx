...
export default function RepairDetailModal({ record, item, onClose }: RepairDetailModalProps) {
  ...
  const idx = repairFlowIndex(record.repair_status);
  const isCollected = record.repair_status === 'collected';
  const isReady = record.repair_status === 'completed';
  const next = nextRepairStatus(record.repair_status);
  const prev = prevRepairStatus(record.repair_status);
  ...
  const advance = async () => {
    ...
    setBusy(true);
    try {
      await updateRepairStatus(record.id, next);
    } finally {
      setBusy(false);
    }
  };
  ...
  const stepBack = async () => {
    ...
    setBusy(true);
    try {
      await updateRepairStatus(record.id, prev);
    } finally {
      setBusy(false);
    }
  };
...
<div className="flex shrink-0 items-start justify-between gap-3 border-b border-shell-line px-5 py-4">
          ...
          <div className={cn(modalSheetBodyScroll, 'px-5 py-4')}>
            ...
            <div className="mt-3 flex items-center gap-2">
              <Badge className={STATUS_BADGE[record.repair_status]}>
            ...
            <div className="mt-4 divide-y divide-shell-line rounded-lg border border-shell-line">
              <DetailRow label="Issue" value={record.issue_description} />
              <DetailRow label="Sent" value={new Date(record.date_sent).toLocaleDateString('en-NG')} />
              {record.expected_return_date ? (
                <DetailRow
                  label="Expected back"
                  value={new Date(record.expected_return_date).toLocaleDateString('en-NG')}
                />
              ) : null}
              {record.engineer_phone ? <DetailRow label="Shop phone" value={record.engineer_phone} mono /> : null}
              {(record.repair_cost ?? 0) > 0 ? (
                <DetailRow label="Quoted cost" value={formatCurrency(record.repair_cost!)} mono />
              ) : null}
            </div>

            {isCollected ? (
              <div className="mt-4 flex gap-3 rounded-lg border border-emerald-500/25 bg-emerald-500/10 p-3.5">
                ...
                <p className="text-sm leading-relaxed text-shell-ink">
                  ...
                  {record.date_returned
                    ? ` on ${new Date(record.date_returned).toLocaleDateString('en-NG')}`
                    : ''}
                  .
                </p>
              </div>
              ...
              <div className="mt-4">
                <div className="flex gap-2">
                  <a
                    ...
                    className={cn('flex-1', !record.engineer_phone && 'pointer-events-none opacity-50')}
                  >
                    <Button
                      variant="outline"
