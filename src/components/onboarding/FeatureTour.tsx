...
const ITEMS = [
  ...
  { Icon: Wrench, title: 'Repairs', text: 'Send units to engineers and track status until they return.' },
  ...
  return (
    <ul className="space-y-3 text-left">
      {ITEMS.map(({ Icon, title, text }) => (
        <li key={title} className="flex gap-3 rounded-xl border border-border bg-white p-3">
